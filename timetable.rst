...
시간표들을 처리하는 API는 시간표의 정보가 포함된 객체 혹은 이것의 배열들을 사용한다. 시간표 객체에는 다음과 같은 속성이 있다.
   ...
   * `id` – 시간표의 ID
            :http:post:`/api/users/(user_id)/timetables` 에서는 
            이 속성이 있으면 해당 시간표 ID의 시간표를 추가하고, 
            없으면 새로운 시간표를 생성하여 추가한다.
   * `name` – 시간표의 이름
   ...
   * `classes` – 시간표에 포함된 강좌의 목록. :ref:`class-object` 의 배열로 이루어져 있다.
                 단, :http:post:`/api/users/(user_id)/timetables` 에서는 :ref:`class-object` 의 속성 중에서 
...
.. http:get:: /api/users/(user_id)/timetables
   ...
   **요청 예시**:
   ...
   .. sourcecode:: http
      ...
      GET /api/users/me/timetables HTTP/1.1
      Host: example.com
      Accept: application/json, text/javascript

   :param user_id: 유저의 ID. `me` 일경우 자기 자신을 나타냄
...
.. http:get:: /api/timetables/(tt_id)
   ...
   ID가 `tt_id` 인 시간표. 외부에서 링크를 통해 접속할 때 사용
...
.. http:get:: /api/users/(user_id)/timetables/(tt_id)
   ...
   사용자 ID가 `user_id` 인 사용자가 보관하고 있는, 시간표 ID가 `tt_id` 인 시간표를 읽음
...
.. http:post:: /api/users/(user_id)/timetables
   ...
   사용자 ID가 `user_id` 인 사용자가 보관하고 있는 시간표의 목록에 새로운 시간표를 추가. Request body의 JSON에 `id` 속성이 있는지 없는지에 따라 동작이 달라진다.
...
.. http:put:: /api/users/(user_id)/timetables/(tt_id)
   ...
   **요청 예시**:
   ...
   .. sourcecode:: http
      ...
      PUT /api/users/me/timetables HTTP/1.1
      Host: example.com
      Content-Type: application/json
   ...
   **응답 예시**:
   ...
   .. sourcecode:: http
      ...
      HTTP/1.1 200 OK
      Location: http://example.com/api/users/me/timetables/123

   :resheader Location: 시간표가 성공적으로 수정되었을 때, 수정된 시간표의 링크
...
.. http:delete:: /api/users/(user_id)/timetables/(tt_id)?from_list=true
   ...
   **요청 예시**:
   ...
   :param user_id: 유저의 ID. `me` 일경우 자기 자신을 나타냄
   :param tt_id: 시간표의 ID
   :param from_list: `true` 이면 사용자의 시간표 목록에서만 삭제 / `false` 이면 시간표 자체를 삭제
